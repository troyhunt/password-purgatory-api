...
class Beelzebub {
  ...
  infuriate = password => {
    ...
    if (
      validChecks.includes(c => c.infuriationLevel === InfuriationLevel.Low)
    ) {
      leastInfuriatingChecks = validChecks.filter(
        c => c.infuriationLevel === InfuriationLevel.Low,
      )
    } else if (
      validChecks.includes(
      ...
      validChecks.includes(c => c.infuriationLevel === InfuriationLevel.High)
    ) {
      leastInfuriatingChecks = validChecks.filter(
    ...
    return leastInfuriatingChecks[
      Math.floor(Math.random() * validChecks.length)
    ]
  }
}
